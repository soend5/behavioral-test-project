...
export async function DELETE(
  ...
  try {
    ...
    const deleted = await prisma.trainingDay.delete({ where: { id: params.id } });

    await writeAudit(prisma, session.user.id, "admin.delete_training_day", "training_day", deleted.id, {
      dayNo: deleted.dayNo,
    });
